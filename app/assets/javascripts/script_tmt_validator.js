...
function tmt_groupValidatorFactory(buttonGroup){
	...
	this.flagValid = function(){
		...
		if(this.errorClass){
			for(var i=0; i<buttonGroup.elements.length; i++){
				buttonGroup.elements[i].className = buttonGroup.elements[i].className.replace(" " + this.errorClass, "");
...
function tmt_filterDemoronizer(str){
	...
	str = str.replace(new RegExp(String.fromCharCode(8364), "g"), "â‚¬");
	return str;
}
...
function tmt_getNodesTable(formNode, type){
	...
	for(var i=0; i<boxNodes.length; i++){
		if(boxNodes[i].getAttribute("type").toLowerCase() == type){
